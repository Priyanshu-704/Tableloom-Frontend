...
export const toServiceResponse = (response, fallback = {}) => {
  ...
  return {
    success: typeof payload?.success === "boolean" ? payload.success : status >= 200 && status < 300,
    message: getApiMessage(payload, fallback.message || ""),
    error: payload?.error || fallback.error || "",
    data: payload?.data !== undefined ? payload.data : fallback.data !== undefined ? fallback.data : null,
    ...
    logoutRequired: payload?.logoutRequired !== undefined ? payload.logoutRequired : fallback.logoutRequired,
    status
  };
};
