...
const buildStorageKey = (audience = "global", subject = "default") =>
  `${STORAGE_PREFIX}:${audience}:${subject || "default"}`;
const getStoredToken = (storageKey) => getStorage()?.getItem(storageKey) || "";
const setStoredToken = (storageKey, token) =>
...
const resolveTokenEndpoint = (audience = "") =>
  String(audience || "").toLowerCase() === "staff"
    ? "/push-notifications/token/staff"
    : "/push-notifications/token/customer";
export const pushNotificationService = {
  buildStorageKey,
  registerToken: async (payload = {}) => {
    try {
      const response = await axiosInstance.post(
        resolveTokenEndpoint(payload?.audience),
        {
          ...payload,
          device: {
            ...buildMetadata(),
  ...
  syncToken: async ({ storageKey, token, ...payload } = {}) => {
    if (!storageKey || !token) {
      return {
        success: false,
        skipped: true,
  ...
  removeStoredToken: async ({ storageKey, token, ...payload } = {}) => {
    if (!storageKey) {
      return {
        success: false,
        skipped: true,
    ...
    const response = await pushNotificationService
      .unregisterToken({
        ...payload,
        token: resolvedToken,
      })
