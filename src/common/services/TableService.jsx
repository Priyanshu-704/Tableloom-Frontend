...
class TableService {
  ...
  getTablesNeedingAttention(tables) {
    return tables.filter(
      (table) =>
        ["maintenance", "cleaning"].includes(table.status) ||
        (table.tokenExpired && table.status !== "inactive"),
    );
  }
  ...
  prepareTableExport(tables) {
    return tables.map((table) => ({
      ...
      "Last Occupied": table.lastOccupied
        ? new Date(table.lastOccupied).toLocaleString()
        : "",
      "Last Cleaned": table.lastCleaned
        ? new Date(table.lastCleaned).toLocaleString()
        : "",
      Notes: table.notes || "",
  ...
  getTableSummary(tables) {
    ...
    return {
      ...
      occupiedCapacity: activeTables
        .filter((t) => t.status === "occupied")
        .reduce((sum, t) => sum + t.capacity, 0),
      tablesNeedingAttention:
        this.getTablesNeedingAttention(activeTables).length,
  ...
  getTablesWithQRIssues(tables) {
    return tables.filter(
      (table) =>
        table.isActive &&
        (table.tokenExpired || !table.qrCode || !table.tokenExpiry),
    );
  }
}
