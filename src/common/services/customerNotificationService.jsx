...
export const customerNotificationService = {
  getNotifications: async (filters = {}) => {
    try {
      ...
      const response = await axiosInstance.get(
        `/notifications/session/${sessionId}`,
        {
          params: filters,
        },
      );
  ...
  markAsRead: async (notificationId) => {
    try {
      ...
      const response = await axiosInstance.put(
        `/notifications/session/${sessionId}/${notificationId}/read`,
      );
      return (
        response?.data ?? {
          success: true,
  ...
  markAllAsRead: async () => {
    try {
      const sessionId = getSessionId();
      const response = await axiosInstance.put(
        `/notifications/session/${sessionId}/mark-all-read`,
      );
      return (
        response?.data ?? {
          success: true,
  ...
  clearAll: async () => {
    try {
      const sessionId = getSessionId();
      const response = await axiosInstance.put(
        `/notifications/session/${sessionId}/clear-all`,
      );
      return (
        response?.data ?? {
          success: true,
