...
export const kitchenStationService = {
  getKitchenStations: async () => {
    try {
      return await kitchenStationRequestCache.run(
  ...
  assignCategoryToStation: async (stationId, categoryId) => {
    try {
      const response = await axiosInstance.put(
        `/kitchen-stations/${stationId}/assign-category/${categoryId}`,
      );
      kitchenStationRequestCache.clear();
      return (
        response?.data ?? {
  ...
  removeCategoryFromStation: async (stationId, categoryId) => {
    try {
      const response = await axiosInstance.delete(
        `/kitchen-stations/${stationId}/remove-category/${categoryId}`,
      );
      kitchenStationRequestCache.clear();
      return (
        response?.data ?? {
  ...
  getStationDashboard: async (stationId) => {
    try {
      const response = await axiosInstance.get(
        `/kitchen-stations/${stationId}/dashboard`,
      );
      return (
        response?.data ?? {
          success: true,
