...
export const buildSuperAdminPath = (path = "/login") =>
  prependAppBasePath(`/super-admin/${String(path || "/login").replace(/^\/+/, "")}`);
...
export const isSuperAdminMonitoringPath = (pathname = "", role = "") =>
  String(role || "").toLowerCase() === "super_admin" && isTenantContextPath(pathname);

export const resolveAdminHomePath = (role = "") => {
  switch (String(role || "").toLowerCase()) {
    case "super_admin":
      return buildAdminPath("/tenant-management");
    case "waiter":
