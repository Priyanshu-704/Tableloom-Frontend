...
function NotificationGroup({ title, items, isReadOnly = false }) {
  const navigate = useNavigate();
  const { activeAction, acknowledge, dismiss, markAsRead } =
    ...
    <section className="space-y-3">
      ...
      {items.map((item) => {
        ...
        const canAcknowledge =
          item.actionRequired &&
          !["acknowledged", "dismissed", "action_taken"].includes(status);
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {!item.isRead ? (
                    <span className="inline-flex items-center gap-1 rounded-full bg-primary-50 px-2 py-1 text-[11px] font-medium text-primary-700">
                ...
                <div className="flex flex-wrap gap-2 text-xs">
                  <Badge
                    className={
                      priorityTone[item.priority] || priorityTone.medium
                    }
                  >
                ...
                <div className="flex flex-wrap gap-3 text-xs text-gray-500">
                  ...
                  {item.metadata?.tableNumber ? (
                    <span>Table {item.metadata.tableNumber}</span>
                  ) : null}
                </div>
              </div>
              ...
              {!isReadOnly ? (
                <Button
                  ...
                  disabled={activeAction === item._id}
                  variant="ghost"
                  size="icon"
                  className="h-8 w-8 cursor-pointer rounded-full text-gray-400 hover:text-gray-600"
  ...
  return (
    <Sheet open={isDrawerOpen} onOpenChange={(open) => !open && closeDrawer()}>
      <SheetContent
        ...
        className="left-2 right-2 top-16 bottom-0 h-auto w-auto max-w-xl rounded-none rounded-t-3xl border-x-0 border-b-0 border-gray-200 bg-[#f7f7f4] p-0 sm:left-3 sm:right-3 sm:top-18 sm:bottom-3 sm:rounded-3xl sm:border sm:max-w-none md:left-auto md:right-0 md:top-16 md:bottom-0 md:h-[calc(100vh-4rem)] md:w-[min(72rem,calc(100vw-14rem))] md:max-w-none md:rounded-none"
      >
        <div className="flex h-full min-h-0 flex-col">
          <div className="border-b border-gray-200 bg-white px-5 py-4">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    className={`${isConnected ? "bg-emerald-100 text-emerald-700" : "bg-sky-100 text-sky-700"}`}
                  >
              ...
              <Button
                ...
                variant="ghost"
                size="icon"
                className="rounded-full text-gray-500 hover:text-gray-700 cursor-pointer"
            ...
            <div className="mt-4 hidden gap-3 md:grid md:grid-cols-3">
              <div className="min-w-34 shrink-0 rounded-2xl bg-slate-50 p-3">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {!isMonitoringMode ? (
                <Button
                  ...
                  disabled={loading || activeAction === "mark-all"}
                  variant="outline"
                  size="sm"
              ...
              {!isMonitoringMode ? (
                <Button
                  type="button"
                  onClick={clearAll}
                  disabled={loading || activeAction === "clear-all"}
                  variant="outline"
                  size="sm"
                  className="cursor-pointer"
                >
                ...
                {loading ? (
                  ...
                  <div className="rounded-2xl border border-dashed border-gray-300 bg-white px-4 py-12 text-center">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      New waiter calls and alerts will appear here
                      automatically.
                    </p>
                  </div>
                ) : (
