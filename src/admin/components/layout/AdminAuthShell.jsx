...
export function AdminAuthShell({
  ...
  const shellClassName = lockViewport
    ? "relative min-h-screen bg-slate-950 text-slate-100 lg:h-screen"
    : "relative min-h-screen bg-slate-950 text-slate-100";
  const layoutClassName = lockViewport
    ? "relative mx-auto grid min-h-screen w-full max-w-7xl items-start gap-8 px-4 py-8 sm:px-6 lg:h-screen lg:grid-cols-[minmax(0,1.05fr)_minmax(0,560px)] lg:gap-12 lg:px-8 lg:py-12"
  ...
  const sidePanelClassName = lockViewport
    ? "order-2 min-h-0 lg:order-1 lg:overflow-hidden lg:pt-6"
    : "order-2 lg:order-1 lg:pt-6";
  const cardWrapClassName = lockViewport
    ? "order-1 mx-auto min-h-0 w-full max-w-xl lg:order-2 lg:max-w-none lg:justify-self-end lg:self-start"
    : "order-1 mx-auto w-full max-w-xl lg:order-2 lg:max-w-none lg:justify-self-end lg:self-start";
  const cardClassName = lockViewport
    ? "flex min-h-0 flex-col rounded-[2rem] border border-white/10 bg-white/95 p-6 shadow-2xl shadow-slate-950/30 backdrop-blur sm:p-8 lg:max-h-[calc(100vh-6rem)] lg:p-10"
    : "rounded-[2rem] border border-white/10 bg-white/95 p-6 shadow-2xl shadow-slate-950/30 backdrop-blur sm:p-8 lg:p-10";
  const contentClassName = contentScrollable
    ? "min-h-0 flex-1 overflow-visible lg:overflow-y-auto lg:pr-1"
    : "";
  useEffect(() => {
  ...
  return (
    <div className={shellClassName}>
      ...
      <div className={layoutClassName}>
        <div
          className={`${sidePanelClassName} ${hideSidePanelOnMobile ? "hidden lg:block" : "block"}`}
        >
        ...
        <div
          className={`${cardWrapClassName} ${hideFormCardOnMobile ? "hidden lg:block" : "block"}`}
        >
          <div className={cardClassName}>
            <div className="mb-8">
              ...
              />
              {mobileAuthMode === "preview" && showMobileForm ? (
                <button
                  type="button"
