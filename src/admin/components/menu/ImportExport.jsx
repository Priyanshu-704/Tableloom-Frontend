...
export function ImportExport() {
  ...
  const downloadTemplate = async () => {
    ...
    try {
      ...
      addNotification(error.response?.data?.message || "Failed to download import template.", "error");
    }
  };
  return <div className="p-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
          ...
          {!isMonitoringMode ? <button onClick={handleExport} disabled={loading} className="flex w-full items-center justify-center rounded-lg bg-green-600 px-4 py-2 text-white transition-colors hover:bg-green-700 disabled:opacity-50">
            {loading ? <div className="mr-2 h-4 w-4 animate-spin rounded-full border-b-2 border-white"></div> : <Download className="mr-2 h-4 w-4" />}
            Export Menu as CSV
          </button> : null}
        </div>
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input id="import-file" type="file" accept=".csv" onChange={e => setImportFile(e.target.files[0])} disabled={isMonitoringMode} className="w-full rounded-lg border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-primary-500" />
            </div>

            <div className="flex space-x-3">
              {!isMonitoringMode ? <button onClick={downloadTemplate} className="flex flex-1 items-center justify-center rounded-lg bg-gray-600 px-4 py-2 text-white transition-colors hover:bg-gray-700">
                <FileText className="mr-2 h-4 w-4" />
                Download Template
              </button> : null}

              {!isMonitoringMode ? <button onClick={handleImport} disabled={loading || !importFile} className="flex flex-1 items-center justify-center rounded-lg bg-blue-600 px-4 py-2 text-white transition-colors hover:bg-blue-700 disabled:opacity-50">
                {loading ? <div className="mr-2 h-4 w-4 animate-spin rounded-full border-b-2 border-white"></div> : <Upload className="mr-2 h-4 w-4" />}
                Import File
              </button> : null}
            </div>
          </div>
      ...
      {importResults && <div className="mt-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
          ...
          {importRows.length > 0 && <div className="mt-6">
              ...
              <div className="overflow-x-auto rounded-lg border border-gray-200">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="divide-y divide-gray-100 bg-white">
                    {importRows.map((row, index) => <tr key={`${row.rowNumber}-${row.itemName || index}`} className="align-top">
          ...
          {importResults.errors && importResults.errors.length > 0 && <details className="mt-4 rounded-lg border border-red-200 bg-red-50 p-4">
              <summary className="cursor-pointer text-sm font-medium text-red-700">
                Raw Error List ({importResults.errors.length})
              </summary>
              <div className="mt-3 space-y-2">
                {importResults.errors.map((error, index) => <div key={index} className="text-sm text-red-700">
                    {error}
                  </div>)}
              </div>
            </details>}
