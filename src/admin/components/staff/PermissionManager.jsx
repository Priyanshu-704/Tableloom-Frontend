...
export function PermissionManager({
  ...
  const savePermissions = async () => {
    try {
      ...
      if (result && result.success) {
        ...
        addNotification(
          result?.message || "Failed to update permissions.",
          "error",
        );
      }
    } catch (error) {
  ...
  const resetPermissions = async () => {
    ...
    try {
      ...
      if (result && result.success) {
        ...
        addNotification(
          result?.message || "Permissions reset to role defaults.",
          "success",
        );
        setTimeout(() => setSuccess(""), 3000);
      } else {
        ...
        addNotification(
          result?.message || "Failed to reset permissions.",
          "error",
        );
      }
    } catch (error) {
  ...
  if (isLoading) {
    return (
      <AdminModal
        ...
        subtitle={`Loading permissions for ${userName}.`}
        onClose={onClose}
        maxWidth="max-w-4xl"
      >
    ...
    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-lg bg-gray-100 px-4 py-2 text-sm text-gray-700 hover:bg-gray-200 disabled:opacity-50 sm:w-auto"
        >
        ...
        <button
          ...
          disabled={isLoading || isSaving}
          className="flex w-full items-center justify-center gap-2 rounded-lg bg-red-100 px-4 py-2 text-sm text-red-700 hover:bg-red-200 disabled:opacity-50 sm:w-auto"
        >
      ...
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:items-center">
        <button
          onClick={onClose}
          disabled={isSaving}
          className="w-full rounded-lg border border-gray-300 px-4 py-2 text-gray-700 hover:bg-gray-50 disabled:opacity-50 sm:w-auto"
        >
          Cancel
        </button>
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-lg bg-primary-600 px-4 py-2 text-white hover:bg-primary-700 disabled:bg-gray-400 disabled:opacity-50 sm:w-auto"
        >
          {isSaving ? (
            <>
  ...
  return (
    <AdminModal
      isOpen={true}
      title="Permission Manager"
      subtitle={`Managing permissions for ${userName} (${userRole}).`}
      onClose={onClose}
      maxWidth="max-w-4xl"
      footer={footer}
    >
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="grid grid-cols-2 gap-2 sm:flex sm:items-center">
            <button
              onClick={selectAll}
              disabled={isLoading || isSaving}
              className="rounded-md bg-blue-100 px-3 py-2 text-xs text-blue-700 hover:bg-blue-200 disabled:opacity-50"
            >
              Select All
            </button>
            <button
              onClick={deselectAll}
              disabled={isLoading || isSaving}
              className="rounded-md bg-gray-100 px-3 py-2 text-xs text-gray-700 hover:bg-gray-200 disabled:opacity-50"
            >
              Deselect All
            </button>
          </div>
      ...
      {error && (
        <div className="mx-4 mt-4 rounded-lg border border-red-200 bg-red-50 p-3 sm:mx-6">
          <p className="text-red-700 text-sm">{error}</p>
        </div>
      )}

      {success && (
        <div className="mx-4 mt-4 rounded-lg border border-green-200 bg-green-50 p-3 sm:mx-6">
              ...
              <div key={category.name} className="mb-6 last:mb-0">
                <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <button
                    ...
                    disabled={isLoading || isSaving}
                    className={`flex w-full items-center justify-center gap-2 rounded-md px-3 py-2 text-sm sm:w-auto ${isFullySelected ? "bg-primary-100 text-primary-700 hover:bg-primary-200" : isPartiallySelected ? "bg-yellow-100 text-yellow-700 hover:bg-yellow-200" : "bg-gray-100 text-gray-700 hover:bg-gray-200"} disabled:opacity-50`}
                  >
                    {isFullySelected ? (
                      <>
                  ...
                  {category.permissions.map((permission) => {
                    ...
                    return (
                      <div
                        ...
                        className={`p-3 rounded-lg border cursor-pointer transition-all ${isSelected ? "border-primary-500 bg-primary-50" : "border-gray-200 hover:border-gray-300 hover:bg-gray-50"} ${isLoading || isSaving ? "opacity-50 cursor-not-allowed" : ""}`}
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <div
                              className={`w-5 h-5 rounded border flex items-center justify-center ${isSelected ? "bg-primary-500 border-primary-500" : "bg-white border-gray-300"}`}
                            >
                              {isSelected && (
