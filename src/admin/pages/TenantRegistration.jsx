...
export function TenantRegistration() {
  ...
  const inputClassName = "mt-2 w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-sky-400 focus:ring-4 focus:ring-sky-100";
  const labelClassName = "block text-sm font-semibold text-slate-800";
  const hintClassName = "mt-2 text-xs leading-5 text-slate-500";
  const routePreview = form.slug && form.key ? buildTenantWorkspacePath({
  ...
  const handleSubmit = async event => {
    ...
    try {
      ...
      setSuccess(response?.message || "Tenant registration submitted successfully");
      setForm(initialForm);
    } catch (submitError) {
  ...
  return <AdminAuthShell contentScrollable mobileAuthMode="formOnly" settings={settings} eyebrow="Platform Onboarding" title="Register Your Restaurant Workspace" description="Submit your restaurant details and the platform team will verify your workspace before admin access is activated." sideTitle="Launch your workspace with a verified setup." sideDescription="Once approved, your tenant admin credentials will be provisioned and you can sign in from the same admin portal." highlights={[{
    title: "One admin experience",
    description: "Tenant admins and platform admins use the same admin panel, with different capabilities."
  }, {
    title: "Verification first",
    description: "New self-registrations stay pending until the platform team reviews them."
  }]}>
      <form className="space-y-4 sm:space-y-5" onSubmit={handleSubmit}>
        ...
        <div className="grid gap-4 sm:grid-cols-2 sm:gap-5">
          <label className={labelClassName}>
            Restaurant Name
            <input className={inputClassName} placeholder="Example: Tableloom Restaurant" value={form.restaurantName} onChange={event => handleChange("restaurantName", event.target.value)} />
          </label>

          <label className={labelClassName}>
            Contact Phone Number
            <input className={inputClassName} placeholder="Example: +91 98765 43210" value={form.phone} onChange={event => handleChange("phone", event.target.value)} />
          </label>
        </div>
        ...
        <div className="rounded-[1.75rem] border border-slate-200 bg-slate-50/80 p-4 sm:p-5">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2 sm:gap-5">
            <label className={labelClassName}>
              ...
              <input className={inputClassName} placeholder="Example: tableloom-restaurant" value={form.slug} onChange={event => handleChange("slug", event.target.value)} />
            ...
            <label className={labelClassName}>
              ...
              <input className={inputClassName} placeholder="Example: main01" value={form.key} onChange={event => handleChange("key", event.target.value)} />
              <p className={hintClassName}>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          <label className={labelClassName}>
            Admin Full Name
            <input className={inputClassName} placeholder="Example: Ayesha Khan" value={form.adminName} onChange={event => handleChange("adminName", event.target.value)} />
          </label>

          <label className={labelClassName}>
            Admin Email Address
            <input className={inputClassName} placeholder="Example: admin@yourrestaurant.com" type="email" value={form.adminEmail} onChange={event => handleChange("adminEmail", event.target.value)} />
          </label>
        </div>
        ...
        <label className={labelClassName}>
          Subscription Plan
          <select className={inputClassName} value={form.subscriptionPlan} onChange={event => handleChange("subscriptionPlan", event.target.value)}>
        ...
        <div className="pb-1 text-center text-sm text-slate-500">
          ...
          <Link className="font-medium text-sky-700 hover:text-sky-800" to={buildAdminPath("/login")}>
            Sign in
          </Link>
        </div>
      </form>
