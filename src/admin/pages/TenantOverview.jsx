            ...
            <div className="space-y-2">
              {columns.map((column) => (
                <div key={column.key} className="flex flex-col gap-1">
                  ...
                  <span className="text-sm text-slate-700 wrap-break-word">
                    {column.render
                      ? column.render(item)
                      : (item?.[column.key] ?? "-")}
                  </span>
...
export function TenantOverview() {
  ...
  if (!overview?.tenant) {
    return (
      <div className="p-4 text-sm text-slate-600 sm:p-6">Tenant not found.</div>
    );
  }
    ...
    <div className="space-y-6 p-4 sm:p-6">
      <div className="flex flex-col gap-4 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm lg:flex-row lg:items-center lg:justify-between sm:p-6">
        <div>
          <button
            ...
            onClick={() =>
              navigate(buildPlatformAdminPath("/tenant-management"))
            }
            type="button"
          >
          ...
          <p className="mt-2 text-sm text-slate-600">
            Route: /{tenant.slug}/{tenant.key} · Plan:{" "}
            {tenant.subscription?.plan || "starter"} · Status:{" "}
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <h2 className="text-xl font-semibold text-slate-900">
          Restaurant Settings Snapshot
        </h2>
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <div className="rounded-2xl bg-slate-50 p-4">
            <div className="text-sm text-slate-500">Restaurant</div>
            <div className="mt-2 font-semibold text-slate-900">
              {settings?.restaurant?.name || tenant.name}
            </div>
            <div className="mt-1 text-sm text-slate-600">
              {settings?.restaurant?.email || tenant.contact?.email || "-"}
            </div>
            <div className="mt-1 text-sm text-slate-600">
              {settings?.restaurant?.phone || tenant.contact?.phone || "-"}
            </div>
          </div>
          <div className="rounded-2xl bg-slate-50 p-4">
            <div className="text-sm text-slate-500">Verification</div>
            <div className="mt-2 font-semibold text-slate-900 capitalize">
              {tenant.onboarding?.verificationStatus || "not_required"}
            </div>
            <div className="mt-1 text-sm text-slate-600">
              Source: {tenant.onboarding?.source || "platform_admin"}
            </div>
            <div className="mt-1 text-sm text-slate-600">
              Submitted:{" "}
              {tenant.onboarding?.submittedAt
                ? new Date(tenant.onboarding.submittedAt).toLocaleString()
                : "-"}
            </div>
          </div>
        </div>
