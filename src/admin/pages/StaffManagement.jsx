...
import { Users, Search, UserPlus, UserCheck, Shield, ChefHat, RefreshCw } from "lucide-react";
...
export function StaffManagement() {
  ...
  const [selectedStaffForRoleUpdate, setSelectedStaffForRoleUpdate] = useState(null);
  ...
  const [selectedStaffForPermissionUpdate, setSelectedStaffForPermissionUpdate] = useState(null);
  const loadStaffMembers = useCallback(async () => {
    try {
      ...
      if (result.success) {
        ...
        addNotification(result.message || "Failed to load staff members", "error");
      }
    } catch (error) {
  ...
  const registerStaff = async staffData => {
    ...
    try {
      ...
      if (result.success) {
        ...
        addNotification(result.message || `Staff member ${staffData.name} registered successfully`, "success");
        return result;
      } else {
        addNotification(result.message || "Failed to register staff member", "error");
        throw new Error(result.message);
      }
    } catch (error) {
      logger.error("Error registering staff:", error);
      addNotification(error.message || "Failed to register staff member", "error");
      throw error;
    } finally {
      setIsLoading(false);
    }
  ...
  const updateStaffRole = async (staffId, newRole) => {
    ...
    try {
      ...
      if (result.success) {
        setStaffMembers(prev => prev.map(staff => staff._id === staffId ? result.data : staff));
        if (selectedStaffForRoleUpdate && selectedStaffForRoleUpdate._id === staffId) {
          setSelectedStaffForRoleUpdate(result.data);
        }
        addNotification(`Staff role updated to ${newRole} successfully`, "success");
        return result;
      } else {
        addNotification(result.message || "Failed to update staff role", "error");
        throw new Error(result.message);
      }
    } catch (error) {
  ...
  const toggleStaffStatus = async (staffId, isActive) => {
    ...
    try {
      ...
      if (result.success) {
        setStaffMembers(prev => prev.map(staff => staff._id === staffId ? result.data : staff));
        addNotification(`Staff member ${isActive ? "activated" : "deactivated"} successfully`, "success");
        return result;
      } else {
        addNotification(result.message || "Failed to update staff status", "error");
        throw new Error(result.message);
      }
    } catch (error) {
      logger.error("Error toggling staff status:", error);
      addNotification(error.message || "Failed to update staff status", "error");
      throw error;
    }
  };
  ...
  const handlePermissionsUpdate = async (userId, newPermissions) => {
    try {
      setStaffMembers(prev => prev.map(staff => staff._id === userId ? {
        ...staff,
  ...
  const deleteStaff = async staffId => {
    ...
    try {
      ...
      if (result.success) {
        ...
        addNotification(result.message || "Failed to delete staff member", "error");
        throw new Error(result.message);
      }
    } catch (error) {
      logger.error("Error deleting staff:", error);
      addNotification(error.message || "Failed to delete staff member", "error");
      throw error;
    }
  };
  ...
  const canManageStaff = () => {
    return !isMonitoringMode && (hasPermission("user_edit") || hasPermission("user_delete") || hasPermission("user_change_status") || hasPermission("user_change_role"));
  };
  ...
  if (isLoading || contextLoading) {
    return <AdminPageSkeleton stats={3} filters={1} cards={6} cardHeight="h-48" />;
  }
  return <div className="space-y-6 p-4 sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex w-full flex-wrap items-center gap-3 sm:w-auto sm:justify-end">
          <Button onClick={loadStaffMembers} disabled={isLoading} variant="outline" title="Refresh staff list">
            <RefreshCw className={`h-5 w-5 ${isLoading ? "animate-spin" : ""}`} />
            <span>Refresh</span>
          </Button>

          {canRegisterStaff() && <Button onClick={handleAddStaff} disabled={isLoading}>
        ...
        <div className="flex flex-col gap-4 md:flex-row">
          <div className="flex-1">
            <div className="relative">
              ...
              <Input type="text" placeholder="Search staff by name or email..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" disabled={isLoading} />
            </div>
          </div>

          <div className="w-full md:w-48">
            <select value={filterRole} onChange={e => setFilterRole(e.target.value)} className="w-full h-10 rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm transition-colors focus:outline-none focus:ring-2 focus:ring-primary-400" disabled={isLoading}>
          ...
          <div className="w-full md:w-48">
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="w-full h-10 rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm transition-colors focus:outline-none focus:ring-2 focus:ring-primary-400" disabled={isLoading}>
      ...
      {staffMembers.length === 0 && <div className="text-center py-12">
          ...
          <p className="mt-2 text-gray-600">
            {searchTerm || filterRole !== "all" || filterStatus !== "all" ? "Try adjusting your search or filters" : "Get started by adding your first staff member"}
          </p>
          {canRegisterStaff() && !searchTerm && filterRole === "all" && filterStatus === "all" && <button onClick={handleAddStaff} disabled={isLoading} className="mt-4 bg-primary-600 hover:bg-primary-700 disabled:bg-gray-400 text-white px-4 py-2 rounded-lg">
                Add Staff Member
              </button>}
        </div>}
      ...
      {!isMonitoringMode && selectedStaffForRoleUpdate && <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={handleCloseRoleUpdate} className="text-gray-400 hover:text-gray-600">
            ...
            <div className="mb-6">
              ...
              <div className="space-y-3">
                {["admin", "manager", "chef", "waiter"].map(role => <label key={role} className={`flex items-center p-3 rounded-lg border cursor-pointer transition-colors ${selectedStaffForRoleUpdate.role === role ? "bg-primary-50 border-primary-500" : "border-gray-200 hover:bg-gray-50"}`}>
                    <input type="radio" name="role" value={role} checked={selectedStaffForRoleUpdate.role === role} onChange={() => {
            ...
            <div className="flex justify-end space-x-3">
              <button onClick={handleCloseRoleUpdate} disabled={isLoading} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 disabled:opacity-50">
                Cancel
              </button>
              <button onClick={() => handleRoleUpdateSubmit(selectedStaffForRoleUpdate.role)} disabled={isLoading || selectedStaffForRoleUpdate.role === staffMembers.find(s => s._id === selectedStaffForRoleUpdate._id)?.role} className="px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 disabled:bg-gray-400">
                {isLoading ? "Updating..." : "Update Role"}
              </button>
            </div>
          </div>
        </div>}
      {!isMonitoringMode && selectedStaffForPermissionUpdate && <PermissionManager userId={selectedStaffForPermissionUpdate._id} userName={selectedStaffForPermissionUpdate.name} userRole={selectedStaffForPermissionUpdate.role} currentPermissions={selectedStaffForPermissionUpdate.permissions || []} onPermissionsUpdate={handlePermissionsUpdate} onClose={handleClosePermissionManager} />}
    </div>;
}
