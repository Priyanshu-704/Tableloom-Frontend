...
import { ShoppingCart, IndianRupee, Users, Clock, ConciergeBell, Utensils, ChefHat } from "lucide-react";
...
const getDashboardBootstrapData = async () => {
  ...
  if (!dashboardBootstrapPromise) {
    dashboardBootstrapPromise = dashboardService.getOverview().then(response => {
...
export function Dashboard() {
  ...
  const [dashboardPayload, setDashboardPayload] = useState(defaultDashboardPayload);
  useEffect(() => {
    let mounted = true;
    const loadDashboard = async (silent = false) => {
      try {
        ...
        const overviewResponse = silent ? await dashboardService.getOverview() : await getDashboardBootstrapData();
        if (!mounted) {
          return;
        }
        setDashboardPayload(normalizeDashboardPayload(overviewResponse?.data || {}));
      } catch {
        if (!silent) {
  ...
  const orderStats = dashboardPayload?.orderStats || defaultDashboardPayload.orderStats;
  const customerAnalytics = dashboardPayload?.customerAnalytics || defaultDashboardPayload.customerAnalytics;
  const feedbackDashboard = dashboardPayload?.feedbackDashboard || defaultDashboardPayload.feedbackDashboard;
  const waiterDashboard = dashboardPayload?.waiterDashboard || defaultDashboardPayload.waiterDashboard;
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm
         sm:p-6">
          ...
          <div className={`overflow-y-auto overscroll-contain pr-1 ${recentActivity.length ? "space-y-4" : ""}`}>
