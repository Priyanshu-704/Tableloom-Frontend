...
const getDefaultReportTitle = (reportType) =>
...
export function Analytics() {
  ...
  useEffect(() => {
    const loadAnalytics = async () => {
      try {
        setLoading(true);
        const { orderFilters, sessionFilters, kitchenFilters } =
  ...
  const overviewCards = useMemo(() => {
    ...
    return [
      ...
      },
      {
        title: "Orders",
        value: Number(
          orderStats?.todayOrders || dashboardStats?.todayOrders || 0,
        ).toLocaleString(),
      ...
      },
      {
        ...
        value: Number(
          sessionStats?.activeSessions || dashboardStats?.activeSessions || 0,
        ).toLocaleString(),
      ...
      },
      {
        ...
        subtitle: feedbackStats?.totalFeedback
          ? `${feedbackStats.totalFeedback} feedback records`
  ...
  const operationsRows = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        value: Number(
          waiterStatistics?.pendingCalls ?? waiterStats?.pendingCalls ?? 0,
        ).toLocaleString(),
      },
    ];
  }, [analytics]);
  const financePaymentMethodData = useMemo(
    () =>
      (analytics?.finance?.paymentMethods || [])
        .map((row, index) => ({
  ...
  const financeOrderTypeData = useMemo(
    () =>
      (analytics?.finance?.orderTypes || [])
        .map((row, index) => ({
          name: row?.orderType
  ...
  const financeBreakdownRows = useMemo(
    () => [
      ...
      [
        "Service Charge",
        formatCurrency(
          analytics?.finance?.summary?.serviceCharge || 0,
          currency,
        ),
      ],
      [
        "Discounts",
        formatCurrency(
          analytics?.finance?.summary?.discountAmount || 0,
          currency,
        ),
      ],
      [
        "Session Revenue",
        formatCurrency(
          analytics?.finance?.summary?.totalSessionRevenue || 0,
          currency,
        ),
      ],
      [
        "Avg Session Revenue",
        formatCurrency(
          analytics?.finance?.summary?.averageSessionRevenue || 0,
          currency,
        ),
      ],
    ],
  ...
  const reportDateLabel = useMemo(
    () => buildRangeLabel(reportForm.startDate, reportForm.endDate),
  ...
  const handleGenerateReport = async () => {
    if (isMonitoringMode) {
      addNotification(
        "Report generation is disabled in monitoring mode.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      const response = await reportService.generateAnalyticsReport(
        {
          ...
          reportTitle: normalizeReportTitle(
            reportForm.title,
      ...
      link.download = getDownloadFilename(
        response?.headers,
        `${reportForm.reportType}-report.${reportForm.format === "excel" ? "xlsx" : "pdf"}`,
      );
      ...
      addNotification(
        `${reportForm.reportType === "finance" ? "Finance" : "Analytics"} report generated successfully`,
        "success",
      );
    } catch (error) {
      addNotification(
    ...
    <div className="space-y-6 p-4 sm:p-6">
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`flex items-center justify-center gap-2 rounded-xl border px-3 py-2.5 text-sm font-medium transition-colors sm:justify-start sm:rounded-none sm:border-x-0 sm:border-t-0 sm:border-b-2 sm:px-1 sm:py-4 ${activeTab === tab.id ? "border-primary-500 bg-primary-50 text-primary-600 sm:bg-transparent" : "border-gray-200 text-gray-500 hover:text-gray-700 sm:border-transparent"}`}
                >
          ...
          {activeTab === "overview" ? (
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              <MetricCard
                ...
                icon={Users}
                items={[
                  ["Total sessions", analytics?.sessions?.totalSessions || 0],
                  ["Active sessions", analytics?.sessions?.activeSessions || 0],
                  [
                    "Completed sessions",
                  ...
                  [
                    ...
                    formatMinutes(analytics?.sessions?.averageSessionTime || 0),
                  ],
                ]}
              />
              <MetricCard
                title="Tables & Feedback"
                icon={Calendar}
                items={[
                  ["Available tables", analytics?.tables?.available || 0],
                  ["Occupied tables", analytics?.tables?.occupied || 0],
                  [
                    "Occupancy rate",
                    `${Number(analytics?.tables?.occupancyRate || 0).toFixed(1)}%`,
                  ],
                  [
                    "NPS score",
                    Math.round(analytics?.feedback?.nps?.score || 0),
                  ],
                ]}
              />
            </div>
          ...
          {activeTab === "sales" ? (
            <div className="space-y-6">
              <MetricCard
                ...
                icon={ShoppingCart}
                items={[
                  ["Total orders", analytics?.orders?.totalOrders || 0],
                  ["Pending orders", analytics?.orders?.pendingOrders || 0],
                  ["Preparing orders", analytics?.orders?.preparingOrders || 0],
                  [
                    "Today's revenue",
                    formatCurrency(
                      analytics?.orders?.todayRevenue || 0,
                      currency,
                    ),
                  ],
                ]}
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <MetricCard
                  ...
                  items={[
                    [
                      "Total revenue",
                      formatCurrency(
                        analytics?.finance?.summary?.totalRevenue || 0,
                        currency,
                      ),
                    ],
                    [
                      "Paid orders",
                      Number(
                        analytics?.finance?.summary?.totalPaidOrders || 0,
                      ).toLocaleString(),
                    ],
                    [
                      "Average order value",
                      formatCurrency(
                        analytics?.finance?.summary?.averageOrderValue || 0,
                        currency,
                      ),
                    ],
                    [
                      "Completed sessions",
                      Number(
                        analytics?.finance?.summary?.completedSessions || 0,
                      ).toLocaleString(),
                    ],
                  ]}
                />
                <MetricCard
                  ...
                  icon={TrendingUp}
                  items={financeBreakdownRows}
                />
              </div>
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <DistributionChartCard
                  title="Payment Method Mix"
                  subtitle="Revenue split by payment method instead of a very long day-by-day series."
                  data={financePaymentMethodData}
                  currency={currency}
                />
                <DistributionChartCard
                  title="Order Type Mix"
                  subtitle="Revenue split by dine-in, takeaway, and delivery."
                  data={financeOrderTypeData}
                  currency={currency}
                />
              </div>
            </div>
          ...
          {activeTab === "menu" ? (
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              <MetricCard
                ...
                icon={PieChartIcon}
                items={[
                  ["Total items", analytics?.menu?.totalItems || 0],
                  ["Available items", analytics?.menu?.availableItems || 0],
                  ["Unavailable items", analytics?.menu?.unavailableItems || 0],
                  ["Categories", analytics?.menu?.categoriesCount || 0],
                ]}
              />
              <MetricCard
                ...
                icon={TrendingUp}
                items={[
                  ["Vegetarian", analytics?.menu?.dietary?.vegetarian || 0],
                  [
                    "Non vegetarian",
                    analytics?.menu?.dietary?.nonVegetarian || 0,
                  ],
                  ["Vegan", analytics?.menu?.dietary?.vegan || 0],
                  ["Gluten free", analytics?.menu?.dietary?.glutenFree || 0],
                ]}
              />
            </div>
          ) : null}
          ...
          {activeTab === "operations" ? (
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              <MetricCard
                ...
                icon={Clock}
                items={operationsRows.map((row) => [row.metric, row.value])}
              />
              <MetricCard
                ...
                items={[
                  ...
                  [
                    "Trending topics",
                    analytics?.feedback?.trendingTopics?.length || 0,
                  ],
                  [
                    "Pending waiter calls",
                    analytics?.waiterCalls?.pendingCalls || 0,
                  ],
                  [
                    "Average response time",
                    formatMinutes(analytics?.waiterCalls?.avgResponseTime || 0),
                  ],
                ]}
              />
            </div>
      ...
      {!isMonitoringMode ? (
        <AdminModal
          ...
          subtitle="Choose the report type, period, and format. One backend request will generate and download the file for you."
          onClose={() => {
            if (!exporting) {
              setShowReportDialog(false);
            }
          ...
          footer={
            <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              ...
              <button
                ...
                disabled={exporting}
                className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-primary-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-60 sm:w-auto"
              >
                {exporting ? (
        ...
        >
          <div className="grid grid-cols-1 gap-4 p-5 md:grid-cols-2">
              ...
              <select
                ...
                onChange={(event) =>
                  setReportForm((current) => {
                    ...
                    const currentDefaultTitle = getDefaultReportTitle(
                      current.reportType,
                    );
                    return {
                      ...current,
                      ...
                      title:
                        !current.title.trim() ||
                        current.title === currentDefaultTitle
                          ? getDefaultReportTitle(nextType)
            ...
            <div className="flex items-end">
              <button
                type="button"
                onClick={applyCurrentFinancialYear}
            ...
            <div className="rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-600 md:col-span-2">
              {reportForm.reportType === "finance"
                ? "Finance report will include only income and revenue related cards, charts, and tables for the selected range."
...
function MetricCard({ title, icon: Icon, items = [] }) {
  const iconElement = React.createElement(Icon, {
              ...
              <div className="space-y-2">
                {columns.map((column, cellIndex) => (
                  ...
                  >
                    ...
                    <span className="wrap-break-word text-sm text-slate-900">
                      {row[cellIndex] ?? "-"}
                    </span>
                  </div>
                ))}
          ...
          <thead>
            <tr className="border-b border-gray-200">
              {columns.map((column) => (
                <th
                  key={column}
                  className="py-3 text-left text-sm font-medium text-gray-600"
                >
                  {column}
                </th>
              ))}
