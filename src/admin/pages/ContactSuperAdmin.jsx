...
export function ContactSuperAdmin() {
  ...
  const loadRequests = async () => {
    ...
    try {
      ...
      addNotification(
        error?.message || "Failed to load support requests",
        "error",
      );
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setForm(initialForm);
      addNotification(
        response?.message || "Support request sent successfully",
        "success",
      );
      await loadRequests();
    } catch (error) {
    ...
    <div className="space-y-6 p-4 sm:p-6">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
              Send tenant, access, billing, technical, or account issues
              directly to the platform team. Only tenant admins can use this
              page.
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,420px)_minmax(0,1fr)]">
        <form
          ...
          className="space-y-4 rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-6"
        >
          ...
          <textarea
            ...
            className="min-h-45 w-full rounded-2xl border border-slate-200 px-4 py-3"
            maxLength={2500}
            placeholder="Describe the issue, impact, and any steps already tried."
            required
          />
          <button
            type="submit"
        ...
        <section className="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Track replies and progress for your tenant workspace.
              </p>
            </div>
                  ...
                  >
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span
                            className={`rounded-full px-2.5 py-1 text-xs font-semibold uppercase tracking-[0.14em] ${STATUS_TONE[request.status] || STATUS_TONE.open}`}
                          >
                            {String(request.status || "open").replace(
                              /_/g,
                              " ",
                            )}
                          </span>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2 text-xs text-slate-500">
                      ...
                      {request.updatedBy?.name ? (
                        <span>Last updated by {request.updatedBy.name}</span>
                      ) : null}
                      {request.resolvedAt ? (
                        <span>
                          Resolved{" "}
          ...
          <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            <div className="flex items-start gap-2">
              ...
              <p>
                Include tenant access issues, billing problems, workspace
                activation questions, or technical blockers so the platform team
                can help faster.
              </p>
            </div>
          </div>
