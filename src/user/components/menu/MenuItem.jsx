...
export function MenuItem({ item }) {
  const { addToCart, updateQuantity, removeFromCart, isItemInCart } = useCart({
  ...
  const handleAdd = async (sizeOverride = null) => {
    ...
    try {
      const activeSize =
        sizeOverride || selectedSize || item.sizes?.[0] || null;
    ...
    <>
      <div className="relative overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition-shadow hover:shadow-md">
        <div className="flex flex-col sm:flex-row">
          <div className="h-40 w-full shrink-0 bg-slate-100 sm:h-24 sm:w-24">
            {item.thumbnail || item.image ? (
              ...
              <div className="flex h-full w-full items-center justify-center text-xs font-medium uppercase tracking-[0.18em] text-slate-400">
                No image
              </div>
            )}
          </div>
          ...
          <div className="flex-1 p-4">
            ...
            {item.sizes?.length > 1 && (
              <div className="mb-2">
                <button
                  ...
                  className="flex w-full items-center gap-2 rounded-lg border border-gray-300 bg-gray-50 px-3 py-1.5 text-left"
                >
                  ...
                  <span className="rounded-md p-1 text-gray-600">
                    <ChevronDown
                      className={`h-4 w-4 transition-transform ${showSizeSelector ? "rotate-180" : ""}`}
                    />
                  </span>
                </button>
              </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0">
                ...
                {getOriginalPrice() > getDisplayPrice() ? (
              ...
              {checkingStatus || isSubmitting ? (
                <div className="h-10 w-full rounded-lg bg-gray-100 animate-pulse sm:w-20"></div>
              ) : cartItemInfo.quantity > 0 ? (
                <div className="flex w-full items-center justify-between rounded-xl bg-primary-50 px-3 py-2 sm:w-auto sm:justify-start sm:bg-transparent sm:px-0 sm:py-0 sm:space-x-2">
                  <button
                    onClick={handleDecrement}
                    disabled={isSubmitting}
                    className="p-1 rounded-full bg-primary-100 text-primary-600 hover:bg-primary-200 transition-colors disabled:opacity-50"
                  >
                ...
                <button
                  ...
                  className="w-full rounded-lg bg-primary-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto"
                >
        ...
        {item.activeDiscount?.isActive ? (
          <div className="absolute bottom-2 left-2 rounded-full bg-sky-100 px-2 py-1 text-xs font-medium text-sky-800">
            {item.activeDiscount.type === "percentage"
              ? `${item.activeDiscount.value}% OFF`
              : `Save ${formatPrice(item.activeDiscount.value)}`}
          </div>
        ) : null}
      </div>
            ...
            <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  Select a size to add this item
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="max-h-[70vh] space-y-3 overflow-y-auto px-5 py-5">
              {item.sizes.map((size) => {
                ...
                return (
                  <div
                    key={size.id}
                    className={`flex items-center justify-between rounded-xl border px-4 py-3 ${isActiveSize ? "border-primary-500 bg-primary-50" : "border-slate-200 bg-white"}`}
                  >
                    <button
                      type="button"
                    ...
                    <button
                      ...
                      disabled={isSubmitting}
                      className="ml-4 rounded-lg bg-primary-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-primary-700 disabled:opacity-50"
                    >
                      Add
                    </button>
                  </div>
