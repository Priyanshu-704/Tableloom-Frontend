...
function SessionRequiredRoute({
  ...
  if (!hasSession) {
    return <Navigate to={buildCustomerPath("/")} replace state={{
...
export function ScanRequiredState() {
  ...
  return <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4">
      <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
        ...
        <h1 className="text-xl font-semibold text-slate-900">Scan Table QR To Continue</h1>
        <p className="mt-2 text-sm text-slate-600">
          You need a valid QR scan and an active dining session before accessing the app.
        </p>
      </div>
    </div>;
}
