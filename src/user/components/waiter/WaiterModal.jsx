...
export function WaiterModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 p-0 sm:items-center sm:p-4">
      <div className="no-scrollbar mb-0 mt-0 max-h-[88vh] w-full max-w-md overflow-y-auto rounded-t-3xl bg-white sm:mb-10 sm:mt-25 sm:rounded-2xl">
        ...
        <div className="p-6">
          ...
          <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2">
            {REASON_OPTIONS.map((option) => {
              ...
              return (
                <button
                  key={option.id}
                  onClick={() => handleQuickCall(option.id)}
                  disabled={isSubmitting}
                  className={`rounded-lg border p-4 text-center transition-colors disabled:opacity-50 ${isSelected ? "border-primary-500 bg-primary-50" : "bg-gray-50 border-gray-200 hover:bg-primary-50 hover:border-primary-300"}`}
                >
                  <Icon
                    className={`mx-auto mb-2 h-6 w-6 ${isSelected ? "text-primary-600" : "text-gray-600"}`}
                  />
                  <span
          ...
          <div className="border-t border-gray-200 pt-6">
            ...
            />
            <button
              onClick={handleSelectedReasonSubmit}
              disabled={!selectedReason || isSubmitting}
              className="w-full bg-primary-600 hover:bg-primary-700 disabled:bg-gray-400 text-white font-semibold py-3 px-4 rounded-lg mt-3 transition-colors"
            >
              {isSubmitting
                ? t("sending")
                : selectedReason
                  ? t("sendMessageToWaiter")
                  : t("callWaiter")}
            </button>
          </div>
        </div>
