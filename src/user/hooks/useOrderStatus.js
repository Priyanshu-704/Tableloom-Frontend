...
const normalizeOrder = (order = null) => {
  ...
  const items = Array.isArray(order?.items) ? order.items.map((item, index) => {
    ...
    const unitPrice = Number(item?.unitPrice) || Number(item?.price) || (quantity > 0 ? totalPrice / quantity : 0);
    return {
      ...item,
      id: item?._id || item?.id || `${item?.menuItem?._id || item?.menuItem || "item"}-${index}`,
      name: item?.name || item?.menuItem?.name || "Menu item",
      sizeName: item?.sizeName || item?.sizeId?.name || item?.size?.name || "",
      price: unitPrice,
      unitPrice,
  ...
  return {
    ...
    total: Number(order?.total ?? order?.totalAmount ?? order?.summary?.total ?? 0),
    createdAt: order?.createdAt || order?.orderPlacedAt || order?.updatedAt || "",
...
export function useOrderStatus(orderId) {
  ...
  const refreshOrder = useCallback(async () => {
    ...
    setIsLoading(true);
    const response = orderId ? await orderService.getOrderById(orderId) : await orderService.getOrderBySession(sessionId);
