...
const getResolvedSessionId = (sessionId = "") => {
  ...
  const storedSessionId = window.sessionStorage.getItem("sessionId") || window.localStorage.getItem("sessionId") || TEST_SESSION_ID;
...
export function useWaiterCall() {
  ...
  const resolvedSessionId = useMemo(() => getResolvedSessionId(sessionId), [sessionId]);
  const loadActiveCalls = useCallback(async () => {
    ...
    try {
      ...
      const response = await waiterCallService.getSessionActiveCalls(resolvedSessionId);
      if (response?.success) {
        dispatch({
          type: "SET_ACTIVE_WAITER_CALLS",
  ...
  const callWaiter = useCallback(async (reason = "waiter", customMessage = "") => {
    if (!resolvedSessionId) {
      return {
        success: false,
  ...
  const cancelCall = useCallback(async (callId, reason = "") => {
    ...
    const response = await waiterCallService.cancelCall(callId, resolvedSessionId, reason);
    if (response?.success) {
      dispatch({
  ...
  const activeCall = useMemo(() => Array.isArray(activeWaiterCalls) ? activeWaiterCalls[0] : null, [activeWaiterCalls]);
  return {
    callWaiter,
    cancelCall,
    isCalling,
