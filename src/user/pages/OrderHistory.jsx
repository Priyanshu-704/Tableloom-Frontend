...
export function OrderHistory() {
  ...
  return <div className="min-h-screen bg-gray-50 p-4 pb-24">
      <div className="mx-auto max-w-3xl space-y-4">
        ...
        {orders.length === 0 ? <div className="rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-sm">
            ...
            <button type="button" onClick={() => navigate(buildCustomerPath("/home/menu"))} className="rounded-lg bg-primary-600 px-4 py-2 font-medium text-white hover:bg-primary-700">
              Start Ordering
            </button>
          </div> : orders.map((order, index) => {
        const isCurrent = index === 0 && !["completed", "cancelled"].includes(order?.status);
        return <button key={order?._id || order?.id || order?.orderNumber} type="button" onClick={() => navigate(buildCustomerPath(`/home/order-status/${order?._id || order?.id || ""}`))} className="w-full rounded-2xl border border-gray-200 bg-white p-5 text-left shadow-sm transition-shadow hover:shadow-md">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {isCurrent ? <span className="rounded-full bg-primary-100 px-2.5 py-1 text-xs font-semibold text-primary-700">
                          Current Order
                        </span> : null}
                    </div>
                    <div className="mt-2 flex flex-wrap items-center gap-2 text-sm text-gray-500">
                      ...
                      <span className="flex items-center gap-1">
                        ...
                        {order?.orderPlacedAt || order?.createdAt ? new Date(order.orderPlacedAt || order.createdAt).toLocaleString() : "Just now"}
                      </span>
                    </div>
                  </div>
                ...
                <div className="mt-4 space-y-2">
                  {(order?.items || []).slice(0, 3).map((item, itemIndex) => <div key={item?._id || item?.id || `${order?._id}-${itemIndex}`} className="flex items-center justify-between text-sm">
                      <span className="text-gray-700">
                        {item?.quantity}x {item?.menuItem?.name || item?.name || "Menu item"}
                        {item?.sizeName || item?.size?.name ? ` (${item?.sizeName || item?.size?.name})` : ""}
                      </span>
                      <span className="text-gray-500">
                        {formatPrice(item?.totalPrice || (item?.unitPrice || item?.price) * (item?.quantity || 0))}
                      </span>
                    </div>)}
                  {(order?.items || []).length > 3 ? <p className="text-sm text-gray-500">
