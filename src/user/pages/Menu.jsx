...
const transformMenuItem = (item) => {
  ...
  return {
    ...
    price:
      sizes[0]?.price ||
      getDiscountedPrice(item.prices?.[0]?.price || 0, discount),
    prices: item.prices || [],
    sizes,
...
export function Menu() {
  ...
  useEffect(() => {
    const loadMenuItems = async () => {
      ...
      try {
        ...
        const transformedItems = itemsArray
          .filter((item) => item.isAvailable === true)
          .map(transformMenuItem);
        setMenuItems((current) =>
          page === 1 ? transformedItems : [...current, ...transformedItems],
        );
  ...
  const visibleCategories = useMemo(() => {
    ...
    const knownCategories = Array.isArray(categories)
      ? categories.filter((category) => {
          ...
          return (
            derivedCategories.has(category._id) ||
            derivedCategories.has(category.name)
          );
        })
      : [];
    if (!knownCategories.length) {
      return Array.from(derivedCategories.values()).sort((a, b) =>
        a.name.localeCompare(b.name),
      );
    }
    derivedCategories.forEach((value, key) => {
      const exists = knownCategories.some(
        (category) => category._id === key || category.name === value.name,
      );
      if (!exists) {
    ...
    return knownCategories.sort((a, b) => {
      const orderA = Number.isFinite(a.displayOrder)
        ? a.displayOrder
        : Number.MAX_SAFE_INTEGER;
      const orderB = Number.isFinite(b.displayOrder)
        ? b.displayOrder
        : Number.MAX_SAFE_INTEGER;
  ...
  useEffect(() => {
    ...
    const sections = sortedCategoryKeys
  ...
  const handleCategoryClick = (categoryId) => {
    ...
    const targetCategory =
      categoryId === "all"
        ...
        : visibleCategories.find((category) => category._id === categoryId)
            ?.name;
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 pb-24 pt-3 lg:px-6">
      <div className="grid gap-6 lg:grid-cols-[20rem_minmax(0,1fr)]">
        ...
        <div className="min-w-0">
          <div className="sticky top-28 z-30 -mx-4 space-y-3 border-b border-slate-200 bg-gray-50/95 px-4 pb-3 pt-2 backdrop-blur sm:top-18 lg:top-0 lg:mx-0 lg:px-0">
            <div className="flex flex-col gap-3 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-gray-300 py-3 pl-10 pr-4 focus:border-transparent focus:ring-2 focus:ring-primary-500"
                  disabled={isLoading}
                />
              </div>
              ...
              <button
                ...
                className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-slate-300 px-4 py-3 text-sm font-medium text-slate-700 sm:w-auto lg:hidden"
              >
            ...
            <div className="overflow-x-auto">
              <div className="flex min-w-max items-center gap-2 pb-1">
                <button
                  ...
                  className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${(filters.category === "all" && activeCategoryKey === "all") || (!sortedCategoryKeys.length && filters.category === "all") ? "bg-primary-600 text-white" : "bg-white text-slate-700 ring-1 ring-slate-200 hover:bg-slate-50"}`}
                >
                  All
                </button>
