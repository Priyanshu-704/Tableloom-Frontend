...
const isCouponAvailable = (coupon) => {
  ...
  if (
    coupon.usageLimit &&
    Number(coupon.usageCount || 0) >= Number(coupon.usageLimit || 0)
  ) {
    return false;
  }
  return true;
...
export function Cart() {
  ...
  const { dispatch, tableNumber } = useApp();
  ...
  const currentTableNumber =
    cart?.table?.number || cart?.table?.tableNumber || tableNumber || "1";
  ...
  const isTaxInclusive = Boolean(
    activeSummary?.taxInclusive ?? settings?.taxSettings?.taxInclusive,
  );
  useEffect(() => {
    let mounted = true;
  ...
  const handleIncrement = async (menuItemId, sizeId) => {
    const cartItem = cart?.items?.find(
      (item) =>
        String(item.menuItemId || item._id) === String(menuItemId) &&
  ...
  const handleOrderPlaced = (order) => {
    ...
    navigate(
      buildCustomerPath(`/home/order-status/${order?._id || order?.id || ""}`),
    );
    notify(
    ...
    return (
      <div className="max-w-2xl mx-auto p-4 pt-20">
        <div className="text-center py-12">
          ...
          <button
            onClick={fetchCart}
            className="cursor-pointer bg-primary-600 hover:bg-primary-700 text-white px-6 py-3 rounded-lg font-semibold"
          >
            Retry
          </button>
        </div>
  ...
  return (
    <div className="mx-auto max-w-4xl pb-24">
      <div className="sticky top-28 z-40 border-b border-gray-200 bg-white p-4 sm:top-18 lg:top-0">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-lg font-bold text-primary-900">
                {cart.table.number || cart.table.tableNumber} •{" "}
                {cart.table.name || cart.table.tableName}
              </p>
            </div>
                ...
                <div className="flex-1">
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="ml-0 flex items-center justify-between rounded-xl bg-slate-50 p-2 sm:ml-4 sm:justify-start sm:bg-transparent sm:p-0 sm:space-x-3">
                      <button
                        onClick={() => handleDecrement(menuItemId, sizeId)}
                        className="cursor-pointer p-1 rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200 transition-colors"
                      >
                      ...
                      <button
                        onClick={() => handleIncrement(menuItemId, sizeId)}
                        className="cursor-pointer p-1 rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200 transition-colors"
                      >
                      ...
                      <button
                        onClick={() => handleRemoveItem(menuItemId, sizeId)}
                        className="cursor-pointer p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors ml-2"
                      >
      ...
      <div className="border-t border-gray-200 bg-white p-4">
        <div className="mb-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          {availableCoupons.length > 0 ? (
            <select
              ...
              className="mb-3 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-gray-700 focus:border-transparent focus:ring-2 focus:ring-primary-500"
            >
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <button
              ...
              className="cursor-pointer w-full rounded-lg bg-primary-600 px-4 py-2 font-semibold text-white transition-colors hover:bg-primary-700 disabled:bg-gray-400 sm:w-auto"
            >
              {isApplyingDiscount
                ? "Applying..."
                : activeCouponCode
          ...
          {activeCouponCode ? (
            <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-700">
              <p>
                Active coupon:{" "}
                <span className="font-semibold">{activeCouponCode}</span>
              </p>
              <p className="mt-1 text-xs text-emerald-600">
                Applying a new coupon will replace the current one.
              </p>
            </div>
          ) : null}
        </div>
        ...
        <div className="mb-6">
          ...
          {cart?.summary ? (
            <div className="space-y-2">
              ...
              {cart.summary.tax > 0 && (
                <div className="flex justify-between">
                  <span className="text-gray-600">
                    Tax{" "}
                    {cart.summary.taxRate ? `(${cart.summary.taxRate}%)` : ""}
              ...
              {cart.summary.serviceCharge > 0 && (
                <div className="flex justify-between">
                  <span className="text-gray-600">
                    Service Charge{" "}
                    {cart.summary.serviceChargeRate
                      ? `(${cart.summary.serviceChargeRate}%)`
                      : ""}
                  </span>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <button
            ...
            className="w-full cursor-pointer rounded-lg bg-red-600 px-6 py-4 font-semibold text-white transition-colors duration-200 hover:bg-red-700 disabled:bg-gray-400"
          >
            Clear All
          </button>
          ...
          <button
            ...
            className="w-full cursor-pointer bg-primary-600 hover:bg-primary-700 disabled:bg-gray-400 text-white font-semibold py-4 px-6 rounded-lg transition-colors duration-200 flex items-center justify-center"
          >
            {isSubmitting ? (
              <>
        ...
        <p className="text-xs text-gray-500 text-center mt-2">
          {cart?.table
            ? `Your order will be prepared and served at Table ${cart.table.number} (${cart.table.name})`
            : "Your order will be prepared and served"}
        </p>
        {isTaxInclusive ? (
