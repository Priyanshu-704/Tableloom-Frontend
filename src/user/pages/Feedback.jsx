...
export function Feedback() {
  ...
  const hasSubmittedFeedback = useMemo(() => feedbackList.length > 0, [feedbackList]);
  ...
  return <div className="mx-auto max-w-5xl space-y-6 px-4 py-6 pb-24 sm:px-6">
      <div className="rounded-3xl bg-gradient-to-r from-sky-500 to-cyan-500 p-6 text-white shadow-lg">
        ...
        <p className="mt-2 max-w-2xl text-sm text-orange-50 sm:text-base">
          Share your dining experience once per session so our team can review it properly.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.2fr,0.8fr]">
        <form onSubmit={handleSubmit} className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            {hasSubmittedFeedback ? <span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-2 text-sm font-medium text-emerald-700">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            {Object.entries(form.ratings).map(([key, value]) => <label key={key} className="rounded-2xl border border-gray-200 p-4">
                ...
                <input type="range" min="1" max="5" value={value} onChange={event => handleRatingChange(key, Number(event.target.value))} disabled={hasSubmittedFeedback || saving} className="mt-3 w-full" />
          ...
          <div className="mt-6">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              return <button key={category} type="button" onClick={() => toggleCategory(category)} disabled={hasSubmittedFeedback || saving} className={`rounded-full px-3 py-2 text-sm ${selected ? "bg-primary-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"} disabled:cursor-not-allowed disabled:opacity-60`}>
          ...
          }))} disabled={hasSubmittedFeedback || saving} className="w-full rounded-2xl border border-gray-300 px-4 py-3 focus:border-transparent focus:ring-2 focus:ring-primary-500 disabled:cursor-not-allowed disabled:bg-gray-50" placeholder="Tell us what went well and what we can improve." />
          </div>
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button type="submit" disabled={saving || hasSubmittedFeedback} className="rounded-xl bg-primary-600 px-5 py-3 font-semibold text-white hover:bg-primary-700 disabled:bg-gray-400">
              {saving ? "Saving..." : hasSubmittedFeedback ? "Feedback Submitted" : "Submit Feedback"}
            </button>
          </div>
        </form>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Submitted feedback is read-only for this session.</p>
            </div>
          </div>
